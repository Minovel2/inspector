...
world.events.worldInitialize.subscribe((worldInitializeData) => {
  const property = new DynamicPropertiesDefinition();
  property.defineNumber('insp');
  worldInitializeData.propertyRegistry.registerWorldDynamicProperties(property);
  if (!world.getDynamicProperty('insp'))
  world.setDynamicProperty('insp', 0);
})
...
function toInsp(itemName,loc,playerName,mode,dimension,score = world.getDynamicProperty('insp')) {
    try {
    ...
    if (itemName && coord && playerName) {
        ...
        world.setDynamicProperty('insp', score + 1);
        }
    } catch {}
}
